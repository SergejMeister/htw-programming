...
public class LifeCycleProvider implements ContainerRequestFilter, ContainerResponseFilter, ExceptionMapper<Throwable> {
    ...
    static private final String PERSON_BY_ALIAS_AND_PASSWORD =
    ...
    static public Person authenticate(final String authentication) {
        ...
        final EntityManager em = BROKER_THREAD_LOCAL.get();
        Query query = em.createQuery(PERSON_BY_ALIAS_AND_PASSWORD);
        query.setParameter("alias", username);
        query.setParameter("passwordHash", passwordHash);
        List<Person> resultPersons = query.getResultList();
        if (resultPersons.size() == 1) {
            return resultPersons.get(0);
        }
